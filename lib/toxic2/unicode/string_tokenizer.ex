# Based on String.Tokenizer from elixir project

# Original code licensed under Apache-2.0
# Copyright 2021 The Elixir Team
# Copyright 2012 Plataformatec

defmodule Toxic2.String.Tokenizer do
  @moduledoc false
  ...
  def tokenize(<<head::utf8, tail::binary>>) do
    cond do
      ascii_upper?(head) ->
  ...
  defp continue(<<?!, tail::binary>>, acc, length, ascii_letters?, scriptset, special) do
  ...
  defp continue(
    ...
    cond do
      ...
      # ' \\\t\n\r!"#$%&\'()*+,-./:;<=>?@[]^`{|}~'
      head <= 127 ->
        {acc, bin, length, ascii_letters?, scriptset, special}
  ...
  # Tolerance (Toxic2 addition): upstream assumes valid UTF-8, but the tolerant lexer may feed a
  # truncated/invalid byte (`αβ` cut mid-codepoint). Such a byte is not part of the identifier —
  # stop and hand the remaining bytes back so the lexer reports them (totality: never raise).
  defp continue(<<_byte, _::binary>> = bin, acc, length, ascii_letters?, scriptset, special) do
    {acc, bin, length, ascii_letters?, scriptset, special}
  end
  ...
  defp validate({acc, rest, length, true, _scriptset, special}, kind) do
    ...
    {kind, :lists.reverse(acc) |> :unicode.characters_to_binary(), rest, length, true, special}
  end
